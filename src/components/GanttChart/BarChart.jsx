...
export default function BarChart({ metrics }) {
  const maxVal = Math.max(...metrics.map((m) => Math.max(m.waitingTime, m.turnaroundTime))) || 1
      ...
      <div className="barchart-body">
        {metrics.map((m, idx) => (
          <div key={m.pid} className="bc-row">
            <div className="bc-pid" style={{ color: colors[idx % colors.length] }}>
              {m.pid}
            </div>
            <div className="bc-bars">
              <div className="bc-bar-wrap" title={`Waiting: ${m.waitingTime}`}>
                <div className="bc-bar wt-bar" style={{ width: `${(m.waitingTime / maxVal) * 100}%` }} />
                <span className="bc-bar-label">{m.waitingTime}</span>
              </div>
              <div className="bc-bar-wrap" title={`Turnaround: ${m.turnaroundTime}`}>
                <div className="bc-bar tat-bar" style={{ width: `${(m.turnaroundTime / maxVal) * 100}%` }} />
